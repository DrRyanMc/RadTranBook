...
def _infer_method_label(results_dir):
    ...
    if tag.startswith("sn"):
        parts = tag.split("_")
        sn = next((p for p in parts if p.lower().startswith("s") and p[1:].isdigit()), None)
        return rf"MG S$_{sn[1:]}$" if sn else r"MG S$_N$"
...
def fig_spectrum_map(snapshots, out_dir, snap_time=1.0):
    ...
    # Keep the colorbar tick labels on the right of the bar so they do not
    # overlap the colormap panel.
    ...
    # Causal radiation wavefront: the farthest a signal launched from the source
    # at r = R_S can travel by time t, r = R_S + c t.
    r_front = R_S + C_LIGHT * t
    if r_edges[0] <= r_front <= r_edges[-1]:
        ax0.axvline(r_front, color="crimson", lw=1.1, ls="-.")
    ...
    if r[0] <= r_front <= r[-1]:
        ax1.axvline(r_front, color="crimson", lw=1.1, ls="-.",
                    label=r"$R_S + c\,t$")
    ax1.set_xlabel("r  (cm)", fontproperties=font)
    ax1.set_ylabel("temperature  (keV)", fontproperties=font)
    ax1.legend(prop=font, fontsize=9, ncol=2, edgecolor="none", facecolor="white",
               framealpha=1.0, columnspacing=1.0, handletextpad=0.4,
               bbox_to_anchor=(0.02, 0.02), loc="lower left")
    # tight_layout is incompatible with the manual GridSpec colorbar column and
    # pushes the colorbar tick labels over the plot; use subplots_adjust instead.
    fig.subplots_adjust(left=0.10, right=0.90, top=0.96, bottom=0.10)
