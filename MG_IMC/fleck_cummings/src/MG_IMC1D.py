...
# Numba cache artifacts can embed the top-level module name "compton_kernel"
# from script-style runs.  Provide a stable alias when imported as a package.
...
def _sample_wien_spectrum_mg(n, T, energy_edges):
    """Sample energy packets from a Wien spectrum and assign groups."""
    if n <= 0:
        return np.array([]), np.array([], dtype=np.int32)
    energies = np.random.gamma(shape=4.0, scale=float(T), size=n)
    groups = np.searchsorted(energy_edges, energies, side="right") - 1
    groups = np.clip(groups, 0, len(energy_edges) - 2).astype(np.int32)
...
def _move_sph_slice_mg(weights, mus, times, positions, cell_indices, groups,
                       mesh, sigma_a, sigma_s, sigma_s_true, group_centers,
                       photon_energies,
                       energy_edges, material_temperature, use_compton,
                       n_groups, dt, refl,
    ...
    for i in range(N):
        ...
        while distance_to_census > 1e-15:
            ...
            if evt == 1:
                ...
                stats[2] += 1
                if use_compton and sigma_s_true[g, loc] > 0.0:
                    if sigma_s[g, loc] > 0.0 and random.random() < sigma_s_true[g, loc] / sigma_s[g, loc]:
                        old_energy = photon_energies[i]
                        dx = math.sqrt(max(0.0, 1.0 - mus[i] * mus[i]))
                        new_energy, _, _, new_dz, material_deposit = compton_scatter_3d(
                            old_energy, dx, 0.0, mus[i], material_temperature[loc]
                        )
                        material_deposit_weight = weights[i] * material_deposit / old_energy
                        dep[g, loc] += material_deposit_weight / ((4.0 / 3.0) * math.pi *
                                                                   (mesh[loc, 1]**3 - mesh[loc, 0]**3))
                        weights[i] *= new_energy / old_energy
                        photon_energies[i] = new_energy
                        new_group = np.searchsorted(energy_edges, new_energy) - 1
                        new_group = max(0, min(n_groups - 1, new_group))
                        groups[i] = new_group
                        g = new_group
                        mus[i] = max(-1.0, min(1.0, new_dz))
            else:
                stats[3] += 1
...
def _move_particles_spherical_mg(weights, mus, times, positions, cell_indices, groups,
                                  mesh, sigma_a, sigma_s, sigma_s_true,
                                  group_centers, photon_energies, energy_edges,
                                  material_temperature,
                                  use_compton, n_groups, dt, refl, weight_floor,
                                  stats_accum):
    ...
    for t in prange(n_threads):
        ...
        if start < end:
            _move_sph_slice_mg(
                ...
                mesh, sigma_a, sigma_s, sigma_s_true, group_centers,
                photon_energies[start:end],
                energy_edges,
                material_temperature, use_compton, n_groups, dt, refl, weight_floor,
...
def _equilibrium_sample_spherical_mg(N, Tr, mesh, energy_edges,
                                     T_emit_floor=0.0,
                                     spectrum="planck"):
    ...
    for i in range(I):
        ...
        if n > 0:
            ...
            if spectrum == "wien":
                _, grps = _sample_wien_spectrum_mg(n, float(Tr[i]), energy_edges)
            else:
...
def init_simulation(Ntarget, Tinit, Tr_init, mesh, energy_edges,
    ...
    initial_spectrum : str — ``"planck"`` or ``"wien"`` packet spectrum
    ...
    (weights, mus, times, positions,
     cell_indices, groups) = _equilibrium_sample_spherical_mg(
        N_ic, Tr_init, mesh, energy_edges,
        T_emit_floor=T_emit_floor, spectrum=initial_spectrum)
...
def step(state, Ntarget, Nboundary, Nsource, Nmax,
    ...
    sigma_s_funcs : optional list of n_groups callables for true scattering
                    opacity σ_s,g(T).  These opacities are added to the
                    Fleck-modified effective scattering opacity.
    use_compton   : if True, events selected from sigma_s_funcs use the
                    explicit thermal Compton kernel and remap groups.
    kompaneets_options : dict or None — if supplied, omit explicit Compton
                    kinematics during transport and apply a cell-local,
                    implicit Kompaneets update to census particles.  Required
                    entry: ``thomson_opacity``. ``sigma_s_funcs`` can retain
                    elastic isotropic scattering during transport. Optional entries are
                    ``group_mean_energy_kev``, ``tolerance``,
                    ``max_iterations``, and
                    ``induced_scattering``.
    ...
    sigma_s_true = np.zeros((n_groups, I))
    ...
    sigma_s_f = (sigma_a_true * (1.0 - f)[np.newaxis, :] + sigma_s_true)
    ...
    if _N_inner > 0 and T_inner > 0.0:
        ...
        if bs is not None:
            ...
            photon_energies = np.concatenate([photon_energies, group_centers[bg]])
    ...
    if _N_src > 0 and np.max(source_arr) > 0.0:
        ...
        if len(sp[0]) > 0:
            ...
            photon_energies = np.concatenate([photon_energies, group_centers[sg.astype(np.int32)]])
    ...
    if len(ew) > 0:
        ...
        photon_energies = np.concatenate([photon_energies, group_centers[eg.astype(np.int32)]])
    ...
    dep, si, boundary_loss_by_group = _move_particles_spherical_mg(
        ...
        np.ascontiguousarray(sigma_s_true, dtype=np.float64),
        group_centers,
        np.ascontiguousarray(photon_energies, dtype=np.float64),
        np.ascontiguousarray(energy_edges, dtype=np.float64),
    ...
    if use_scalar_intensity_Tr:
        ...
        path_radiation_temperature = radiation_temperature_new.copy()
    else:
        path_radiation_temperature = np.zeros(I)
    ...
    groups       = groups[keep]
    photon_energies = photon_energies[keep]

    # -----------------------------------------------------------------------
    # 8b. Implicit Kompaneets frequency redistribution of census particles
    # -----------------------------------------------------------------------
    ...
    if kompaneets_options is not None:
        ...
        (weights, groups, photon_energies, internal_energy, temperature,
         kompaneets_diagnostics) = implicit_kompaneets_census_update(
            weights, cell_indices, groups, photon_energies, volumes,
            internal_energy, temperature, dt, energy_edges, thomson_opacity,
            __c, inv_eos, group_mean_energy_kev=mean_energy,
            tolerance=tolerance, max_iterations=max_iterations,
            induced_scattering=induced_scattering,
            group_scalar_capacity=scalar_capacity,
        )
        # The path-length estimator applies to the pre-Kompaneets transport
        # state.  Census energy is the appropriate end-of-step estimator.
        valid = (cell_indices >= 0) & (cell_indices < I) & (weights > 0.0)
        radiation_energy = np.bincount(
            cell_indices[valid].astype(np.int64), weights=weights[valid],
            minlength=I,
        ) if np.any(valid) else np.zeros(I)
        radiation_temperature_new = (radiation_energy / volumes / __a) ** 0.25

    census_radiation_temperature = radiation_temperature_new.copy()
    ...
    if _do_comb:
        ...
        # The group-wise combiner does not preserve within-group photon
        # energies, so initialize surviving comb particles at their centers.
        photon_energies = group_centers[groups]
    ...
    postcomb_radiation_temperature = (
        np.sum(radiation_energy_by_group, axis=0) / volumes / __a
    ) ** 0.25
    ...
    info = {
        ...
        "radiation_temperature_pathlength": path_radiation_temperature.copy(),
        "radiation_temperature_census": census_radiation_temperature.copy(),
        "radiation_temperature_postcomb": postcomb_radiation_temperature.copy(),
...
def run_simulation(Ntarget, Nboundary, Nsource, Nmax,
                   ...
                   sigma_s_funcs=None, use_compton=False,
                   kompaneets_options=None):
