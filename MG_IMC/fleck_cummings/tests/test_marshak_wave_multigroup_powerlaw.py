...
def _planck_group_integral_fallback(E_low, E_high, T):
    ...
    raise ValueError("Planck integrals library not available; fallback used.")
    if T <= 0.0:
        return 0.0
    nquad = 80
...
def powerlaw_opacity_at_energy(T, E, rho=1.0):
    ...
    return np.minimum(10.0 * rho * (T_use ** -0.5) * (E ** -3.0), 1e7)
...
def run_marshak_wave_multigroup_powerlaw_imc(
    ...
    x_max = Lx
    ny = 1
    if nominal_target_times is None:
        nominal_target_times = [0.1, 0.5, 1.0, 2.0, 5.0, 10.0]
    target_times = [t for t in nominal_target_times if t <= final_time + 1e-12]
    ...
    #print target times
    print(f"  Target times for output snapshots (ns): {target_times}")
    if nmax_growth > 0:
    ...
    # Build the set of times at which we must save to history:
    # user target_times + every 0.5 ns + final_time.
    save_times = sorted(set(
        list(target_times)
        + [k * 0.5 for k in range(1, int(np.ceil(final_time / 0.5)) + 1)]
        + [final_time]
    ))
    save_times = [s for s in save_times if s <= final_time + 1e-12]
    _next_save_idx = 0  # index into save_times for the next snapshot to capture
    print(f"  History save times (ns): {save_times}")
    ...
    if restart_from is not None:
        ...
        if abs(float(saved_meta.get("dt", dt)) - float(dt)) > 1e-14:
            ...
            print("Warning: Checkpoint dt does not match current --dt. Using checkpoint dt.")
    ...
    # Advance _next_save_idx past times already covered (e.g. on restart)
    while _next_save_idx < len(save_times) and t >= save_times[_next_save_idx] - time_tol:
        _next_save_idx += 1
    n_steps_total = int(np.ceil((final_time - t) / dt))
    # Adaptive time step: start at dt and grow geometrically by dt_grow each step,
    # capped at dt_max. dt_grow=1.0 (default) reproduces the fixed-dt behavior.
    dt_cap = float(dt if dt_max is None else dt_max)
    current_dt = float(dt)
    adaptive = (dt_grow > 1.0 + 1e-12) and (dt_cap > dt + 1e-15)
    throttle = adaptive and (dt_max_dT > 0.0)
    if adaptive:
        msg = f"  Adaptive dt: start {dt} ns, grow x{dt_grow}/step, cap {dt_cap} ns"
        if throttle:
            msg += f", throttled to max per-step dT/Tpeak <= {dt_max_dT:.3f}"
        print(msg)
    ...
    while t < final_time - time_tol:
        dt_step = min(current_dt, final_time - t)
        # Cut dt short to land exactly on the next save time
        if _next_save_idx < len(save_times):
            dt_to_save = save_times[_next_save_idx] - t
            if dt_to_save > time_tol and dt_to_save < dt_step:
                dt_step = dt_to_save
        if dt_step <= time_tol:
            break

        T_prev = np.asarray(state.temperature, dtype=float).copy() if throttle else None
        ...
        if adaptive:
            if throttle:
                # Throttle growth by the observed max per-step relative change in
                # material temperature (normalized by the global peak). Where the
                # wave is moving fast the change is large -> dt is held down; where
                # the solution has equilibrated dt grows geometrically toward dt_cap.
                T_now = np.asarray(state.temperature, dtype=float)
                dT = float(np.max(np.abs(T_now - T_prev)))
                T_scale = max(float(np.max(T_now)), 1e-30)
                dT_rel = dT / T_scale
                if dT_rel > 1e-30:
                    grow_factor = dt_max_dT / dT_rel
                else:
                    grow_factor = dt_grow
                # Never grow faster than dt_grow; never shrink by more than 2x/step.
                grow_factor = min(grow_factor, dt_grow)
                grow_factor = max(grow_factor, 0.5)
                current_dt = min(max(current_dt * grow_factor, dt), dt_cap)
            else:
                current_dt = min(current_dt * dt_grow, dt_cap)
        ...
        # Save to history when we cross (or reach) the next scheduled save time,
        # or at the very end of the simulation.
        should_save = (final_time - t) < time_tol
        if not should_save and _next_save_idx < len(save_times):
            # Save if current time is at or past the next save time
            if t >= save_times[_next_save_idx] - time_tol:
                should_save = True
        if should_save:
            # Advance past all save_times we've now exceeded
            while _next_save_idx < len(save_times) and t >= save_times[_next_save_idx] - time_tol:
                _next_save_idx += 1
...
def main():
    ...
    parser.add_argument("--Lx", type=float, default=7.0, help="Length of the x-domain (default: 7.0 cm)")
    parser.add_argument("--dt", type=float, default=0.01, help="Timestep in ns (default: 0.01)")
    parser.add_argument("--dt-grow", type=float, default=1.0,
                        help="Geometric growth factor applied to dt each step (1.0 = fixed dt)")
    parser.add_argument("--dt-max", type=float, default=None,
                        help="Cap on the adaptive time step in ns (default: equal to --dt)")
    parser.add_argument("--dt-max-dT", type=float, default=0.0,
                        help="Throttle dt growth so max per-step relative material-T change "
                             "(dT/Tpeak) stays below this target (0 = no throttle)")
    #nominal target times for output snapshots (in ns)
    parser.add_argument("--target-times", type=float, nargs="+", default=[0.1, 0.5, 1.0, 2.0, 5.0, 10.0],
                        help="Target times for output snapshots (ns) (default: 0.1 0.5 1.0 2.0 5.0 10.0)")
    ...
    if args.mode is None:
        ...
        nominal_target_times = [0.1, 0.5, 1.0, 2.0, 5.0, 10.0] if args.target_times is None else args.target_times
    ...
    run_marshak_wave_multigroup_powerlaw_imc(
        ...
        Lx=args.Lx,
        dt=args.dt,
        dt_grow=args.dt_grow,
        dt_max=args.dt_max,
        dt_max_dT=args.dt_max_dT,
        ...
        nominal_target_times=nominal_target_times
    )
