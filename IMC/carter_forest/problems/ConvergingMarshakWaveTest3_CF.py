...
"""
Converging Marshak Wave Test 3 — Carter-Forest IMC, spherical geometry.

Companion to:
  nonEquilibriumDiffusion/problems/converging_marshak_wave_test3.py  (diffusion)
  IMC/ConvergingMarshakWave.py                                        (test 1, IMC)

Uses IMC1D_CarterForest.py with geometry='spherical'.  All physical parameters and the
...
radii computed from the mesh.  IMC1D_CarterForest.py requires each function to
accept a single array argument (T or u, length = n_cells) and return an array
of the same length, which the closures provide.
...
from IMC.carter_forest.src import IMC1D_CarterForest as imc
...
# =============================================================================
# PHYSICAL CONSTANTS  (match IMC1D_CarterForest)
# =============================================================================
...
def plot_results(snapshots, save_prefix='converging_marshak_wave_test3_cf'):
    """
    Save two figures:
...
def run(n_cells=50, Ntarget=5000, Nboundary=2000, NMax=20000,
        ...
        save_prefix='converging_marshak_wave_test3_cf',
    ...
    print('=' * 70)
    print('Converging Marshak Wave Test 3  (CF-IMC / spherical / ρ∝r^{-ω})')
    print('=' * 70)
    print(f'  n_cells={n_cells},  Ntarget={Ntarget},  '
          f'Nboundary={Nboundary},  NMax={NMax}')
    print(f'  dt={dt} ns,  total elapsed={duration:.4f} ns')
    if event_cap_per_particle > 0:
        print(f'  event_cap_per_particle={event_cap_per_particle}')
    if fastpath_threshold > 0.0:
        print(f'  fastpath_threshold={fastpath_threshold}')
    ...
    if profile:
        ...
        event_totals = np.zeros(8, dtype=np.int64)
    ...
    while state.time < duration - 1e-12:
        ...
        if profile and 'profiling' in info:
            ...
            event_totals[2] += events['absorption_continue_events']
            event_totals[3] += events['census_events']
            event_totals[4] += events['absorption_capture_events']
            event_totals[5] += events['weight_floor_kills']
            event_totals[6] += events['reflections']
            event_totals[7] += events['event_cap_hits']
    ...
    if profile and step_count > 0:
        ...
        print(f'  Absorption-continue events: {int(event_totals[2])}')
        print(f'  Census events             : {int(event_totals[3])}')
        print(f'  Absorption-capture events : {int(event_totals[4])}')
        print(f'  Weight-floor kills        : {int(event_totals[5])}')
        print(f'  Reflections               : {int(event_totals[6])}')
        print(f'  Event-cap hits            : {int(event_totals[7])}')
...
def _build_arg_parser():
    parser = argparse.ArgumentParser(
        description='Converging Marshak Wave Test 3 Carter-Forest IMC driver (spherical, ρ∝r^{-ω}).',
    ...
    parser.add_argument('--save-prefix', type=str,
                        default='converging_marshak_wave_test3_cf',
    ...
    parser.add_argument('--checkpoint-path', type=str,
                        default='converging_marshak_wave_test3_cf_checkpoint.pkl',
    ...
    parser.add_argument('--event-cap-per-particle', type=int, default=0,
                        help='Optional max transport events per particle per step (0 disables).')
    parser.add_argument('--fastpath-threshold', type=float, default=0.0,
                        help='Optional CF hybrid shortcut threshold for c*sigma_a*beta*dt_remaining (0 disables).')
    return parser
