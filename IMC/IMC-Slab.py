...
def move_particle(weight, mu, position, cell_l, cell_r,sigma_a, sigma_s, distance_to_census):
    ...
    distance_to_scatter = -math.log(1-random.random())/sigma_t #random.expovariate(sigma_s)
    ...
    if math.fabs(distance_to_boundary - distance_to_next_event) < 1e-10:
        ...
        deposited_intensity = weight/sigma_t/dx
        if (random.random() < sigma_a/sigma_t):
            #absorb particle
            deposited_weight = weight
            weight = 0
            distance_to_census = 0
            return weight, mu, position, new_location, deposited_weight/dx, deposited_intensity/dt, distance_to_census
    else:
        #census
        True
    ...
    # if (sigma_a > 1e-10):
    #     deposited_intensity = 0 #weight*(1-weight_factor)/sigma_a/dx
    # else:
    #     deposited_intensity = weight*distance_to_next_event/dx
    deposited_weight = 0 #deposited_intensity*sigma_a
...
def create_boundary(N,T,dt):
    assert (N > 0)
    total_emission = __a*__c*T**4/2*dt
...
def run_simulation(Ntarget,Nboundary,NMax, Tinit,Tr_init, T_boundary, dt, mesh, 
                   sigma_a_func,eos,inv_eos,cv, final_time, reflect=(False,False),
                   output_freq=1):
    ...
    while time < final_time:
        ...
        if (reflect[0]): #reflecting boundary
            ...
            boundary_loss += np.sum(weights[mask])/__c
        ...
        # weights *= (1+lost_weight/current_weights)
        # mus = mus[mask]
        # times = times[mask]
        # positions = positions[mask]
        # cell_indices = cell_indices[mask]
        ...
        total_radiation_energy = np.sum(weights) #np.sum(a*radiation_temperature**4*dxs)
        total_energy = total_internal_energy + total_radiation_energy
        boundary_emission += 0.5*__a*dt*(T_boundary[0]**4+T_boundary[1]**4)
...
if __name__ == "__main__":
    ...
    final_time = dt*10
    #run simulation
    times, radiation_temperatures, temperatures = run_simulation(Ntarget,Nboundary, NMax,Tinit,Trinit,
                                                                 T_boundary, dt, mesh, sigma_a_f,
                                                                 eos,inv_eos,cv, final_time, reflect=(True,True))
    print(temperatures.shape)
    #plot temperatures in cell 0 over time
    plt.plot(times,temperatures[:,0])
    plt.plot(times,radiation_temperatures[:,0])
    plt.plot(times,times*0+0.43611)
