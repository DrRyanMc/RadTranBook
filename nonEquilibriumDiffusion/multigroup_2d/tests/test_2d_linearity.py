...
"""Quick test to verify 2D B operator linearity after D_faces fix"""

import sys
import numpy as np
from nonEquilibriumDiffusion.multigroup_2d.src.multigroup_diffusion_solver_2d import MultigroupDiffusionSolver2D, C_LIGHT, A_RAD, Bg_multigroup
...
# Setup
print("Setting up 2D multigroup solver...")
n_groups = 10
...
# Test linearity
print(f"\nTesting B operator linearity...")
print(f"Problem size: {solver.n_total} unknowns ({solver.nx_cells}×{solver.ny_cells} cells)")

# Take one step with verbose output to see linearity test
info = solver.step(verbose=True, gmres_tol=1e-6, gmres_maxiter=200, use_preconditioner=False)

print(f"\n{'='*80}")
if info.get('converged', False):
    print(f"✓ Newton converged in {info['newton_iterations']} iterations")
    print(f"✓ Total GMRES iterations: {info.get('total_gmres_iterations', 'N/A')}")
else:
