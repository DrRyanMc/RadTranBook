#!/usr/bin/env python3
"""
Compare Dirichlet vs Marshak boundary conditions for Converging Marshak Wave Test 3.

Usage:
  python compare_bc_test3.py [--force-run] [--n-cells N]

Options:
  --force-run    : Force re-run even if cached solutions exist
  --n-cells N    : Number of cells (default 200)
"""
...
import converging_marshak_wave_test3 as test_dirichlet
import converging_marshak_wave_test3_marshak as test_marshak


def get_cache_path(bc_type, n_cells):
    """Return path for cached solution."""
    return f'cache_test3_{bc_type}_n{n_cells}.pkl'
...
def plot_comparison(solutions_dirichlet, solutions_marshak, save_prefix='compare_bc_test3'):
    """
    ...
    r_anal = np.linspace(1e-8, test_dirichlet.R, 2000)
    ...
    for idx, (sol_d, sol_m, col) in enumerate(zip(solutions_dirichlet, solutions_marshak, colors)):
        ...
        label_base = f't = {t_ns:.2f} ns'
        ax.plot(r_cm_d / 1e-4, T_HeV_d, color=col, lw=2, ls='-', 
                label=f'{label_base} (Dirichlet)')
        ax.plot(r_cm_m / 1e-4, T_HeV_m, color=col, lw=2, ls='--',
    ...
    ax.set_ylabel(r'$T$ (HeV)', fontsize=12)
    ax.set_title('Test 3: Boundary Condition Comparison (Temperature)', fontsize=13)
    ...
    for idx, (sol_d, sol_m, col) in enumerate(zip(solutions_dirichlet, solutions_marshak, colors)):
        ...
        u_d = test_dirichlet.test3_material_energy(sol_d['T_keV'], r_cm_d) * 1e3
        u_m = test_marshak.test3_material_energy(sol_m['T_keV'], r_cm_m) * 1e3
        
        # Analytic
        u_ref = test_dirichlet.u_analytic_per_1e13(r_anal, t_ns)
        
        label_base = f't = {t_ns:.2f} ns'
        ax.plot(r_cm_d / 1e-4, u_d, color=col, lw=2, ls='-',
                label=f'{label_base} (Dirichlet)')
        ax.plot(r_cm_m / 1e-4, u_m, color=col, lw=2, ls='--',
                label=f'{label_base} (Marshak)')
        ax.plot(r_anal / 1e-4, u_ref, color=col, lw=1.5, ls=':',
                label=f'{label_base} (analytic)')
    
    ax.set_xlabel(r'$r$ ($\mu$m)', fontsize=12)
    ax.set_ylabel(r'$u$ ($10^{13}$ erg/cm$^3$)', fontsize=12)
    ax.set_title('Test 3: Boundary Condition Comparison (Energy Density)', fontsize=13)
...
def main():
    parser = argparse.ArgumentParser(
        description='Compare Dirichlet vs Marshak BCs for Test 3'
    )
    ...
    print("=" * 70)
    print("Converging Marshak Wave Test 3: BC Comparison")
    print("=" * 70)
