...
"""
Converging Marshak Wave Test Problem - Spherical Geometry (Test 1) - MARSHAK BC

Spherically convergent radiative heat wave in a uniform gold-like material.
This version uses the Robin/Marshak boundary condition with bath temperature.
...
Boundary conditions:
  ...
  r = R:       Robin/Marshak condition with bath temperature
               φ + (c/3k(T_bath)) dφ/dr = aT_bath⁴
               T_bath = 1.34503465 (-t/ns)^0.0920519 W^(5/8)(ξ_R) V(ξ_R)
               ξ_R = 10/(-t/ns)^0.679501, V(ξ) = 0.4345ξ^{-2.752} + 0.2451ξ^{-1.454}
...
def _Vxsi_scalar(xi):
    """V(ξ) similarity profile for Test 1 (scalar).
...
Vxsi = np.vectorize(_Vxsi_scalar)
...
def _surface_T_keV(t_ns):
    """Surface temperature in keV (interior profile) from self-similar solution."""
    T_HeV = T_analytic_HeV(R, t_ns)
    return max(T_HeV / T_HEV_PER_KEV, 1e-8)


def _Lambda(xi):
    """Λ(ξ) = ξ V(ξ) W^{-1.5}(ξ) for Test 1."""
    W_val = _Wxsi_scalar(xi)
    V_val = _Vxsi_scalar(xi)
    if W_val < 1e-30:
        return 0.0
    return xi * V_val * W_val**(-1.5)


def _bath_T_keV(t_ns):
    """Bath (boundary) temperature in keV from equation (8.82).
    
    T_bath(t) = [1 + 0.103502 Λ(ξ_R(t)) (-t/ns)^{-0.541423}]^{1/4} T_s(t)
    
    where Λ(ξ) = ξ V(ξ) W^{-1.5}(ξ)
    """
    t_clamped = max(float(t_ns), -1e30)
    ...
    # Surface temperature
    T_s = _surface_T_keV(t_clamped)
    
    return T_s * correction
...
def right_bc(phi, x):
    ...
    Standard Marshak BC: φ/2 + (c/3k) dφ/dr = (a c T^4) / 2
    Multiply by 2:       φ + 2(c/3k) dφ/dr = a c T^4
    
    Using corrected bath temperature from equation (8.82).
    """
    T_bath = _bath_T_keV(_t_phys_ns_state[0])
    k_boundary = test1_opacity(T_bath)  # opacity at bath temperature
    beta = 1.0  / (3.0 * k_boundary)  # Robin coefficient (with factor of 2)
    gamma =0.5*A_RAD *C_LIGHT* T_bath**4  # RHS
    return 0.5, beta, gamma  # 1·φ + beta·(dφ/dr) = gamma
...
def run(n_cells=200, dt_initial=0.0001, dt_max=0.5, dt_growth=1.05,
        save_prefix='converging_marshak_wave_marshak', 
        t_duration = T_FINAL_NS - T_INIT_NS): # ≈ 28.626 ns  (positive)
    """
    Run the converging Marshak wave problem with Marshak/Robin boundary condition.
    ...
    while phys_elapsed < t_duration - 1e-12:
        ...
        if step % 50 == 0 or step <= 3:
            ...
            T_bath_HeV = _bath_T_keV(t_now) * T_HEV_PER_KEV
