...
"""
Converging Marshak Wave Test Problem 3 - Spherical Geometry - MARSHAK BC
...
Boundary conditions:
  ...
  r = R:       Robin/Marshak condition with bath temperature
               ...
               T_bath = 1.1982 (-t/ns)^{0.0276392} W^{1/2}(ξ_R) V(ξ_R)
               ξ_R = 10 / (-t/ns)^{1.1157536}
...
def _Vxsi_scalar(xi):
    """V(ξ) similarity profile for Test 3 (scalar).
...
def _bath_T_keV(t_ns):
    """Bath (boundary) temperature in keV from equation (8.88).
    ...
    # Compute the correction factor (Test 3 constants from equation 8.88)
...
def right_bc(phi, x):
    ...
    Using corrected bath temperature from equation (8.88).
    """
    T_bath = _bath_T_keV(_t_phys_ns_state[0])
    k_boundary = test3_opacity(T_bath, R)  # opacity at (T_bath, R)
...
def run(n_cells=200, dt_initial=0.0001, dt_max=0.5, dt_growth=1.05,
        save_prefix='converging_marshak_wave_test3_marshak',
        t_duration=T_FINAL_NS - T_INIT_NS):   # ≈ 6.875 ns  (positive)
    """
    Run the converging Marshak wave Test 3 problem with Marshak/Robin boundary condition.
