...
from nonEquilibriumDiffusion.finite_volume_1d.src.oneDFV import NonEquilibriumRadiationDiffusionSolver
...
# Start with moderate source - 0.5 in normalized units corresponds to a·c/2
...
dt = 0.05 * mean_free_time  # Timestep
...
T_init = 0.001  # keV (not too small to avoid stiffness)
...
print(f"\n{'='*80}")
print("Starting time evolution with built-in source support...")
print(f"{'='*80}")
...
next_output_idx = 0  # Index of next output time to save

# Time evolution loop using built-in solver.time_step() with source
current_time = 0.0
for step in range(n_steps):
    ...
    # Take single timestep with source
    ...
    if next_output_idx < len(all_output_times_mft):
        ...
        if current_time >= target_time:
            ...
            print(f"  Saved solution at τ = {tau_value:.4f} (t = {current_time:.6e} ns)")
            next_output_idx += 1

print(f"\nCompleted {n_steps} steps to t = {current_time:.6e} ns = {current_time/mean_free_time:.3f}τ")
...
#fig1.suptitle('Su-Olson Problem: Early Times', fontsize=16, fontweight='bold') #publication quality figures don't have titles.
...
for idx, tau_val in enumerate(early_output_times_mft):
    ...
    # Plot numerical radiation energy with solid line (limited to x <= 5 cm)
    mask = sol['r'] <= 5.0
    ax1.plot(sol['r'][mask], sol['E_rad'][mask], color=color, linestyle='-', 
            linewidth=2.5, alpha=0.8)
    
    # Plot numerical material energy with dashed line
    ax1.plot(sol['r'][mask], sol['E_mat'][mask], color=color, linestyle='--', 
            linewidth=2.5, alpha=0.8)
    
    # Plot reference data with symbols (filled for radiation, open for material)
    if abs(su_olson_tau[time_index] - tau_val) < 0.01:
        ref_mask = su_olson_x <= 5.0
        # Radiation energy reference (filled symbols)
        ...
        # Material energy reference (open symbols)
...
ax1.set_ylabel(r'Normalized Energy Density $\frac{\phi}{ac T_0^4}$ or $\frac{e(T)}{a T_0^4}$', fontsize=14)

# Create custom legend with just line styles
from matplotlib.lines import Line2D
legend_elements = [Line2D([0], [0], color='black', linestyle='-', linewidth=2.5, label='$E_r$'),
                   Line2D([0], [0], color='black', linestyle='--', linewidth=2.5, label='$e(T)$')]
...
plt.tight_layout()
show('su_olson_early_times.pdf', close_after=True)
print("Saved plot to 'su_olson_early_times.pdf'")
...
#fig2.suptitle('Su-Olson Problem: Late Times', fontsize=16, fontweight='bold') #publication quality figures don't have titles.
...
for idx, tau_val in enumerate(late_output_times_mft):
    ...
    sol = saved_solutions[tau_val]
    color = colors[idx + 3]  # Use different colors
    marker = markers[idx + 3]
    
    # Find corresponding reference data index
    time_index = np.argmin(np.abs(su_olson_tau - tau_val))
    
    # Plot numerical radiation energy with solid line (full domain)
...
ax2.set_ylabel(r'Normalized Energy Density $\frac{\phi}{ac T_0^4}$ or $\frac{e(T)}{a T_0^4}$', fontsize=14)
...
plt.tight_layout()
show('su_olson_late_times.pdf', close_after=True)
print("Saved plot to 'su_olson_late_times.pdf'")

# Summary
print(f"\n{'='*80}")
print("SUMMARY")
print(f"{'='*80}")
print(f"Problem: Su-Olson type test with radiation-dominated material")
...
print(f"Final time: t = 1.0τ = {final_time:.6e} ns")
print(f"Total timesteps: {n_steps}")
print(f"\nExpected behavior:")
print(f"  - Temperature should increase in source region and diffuse outward")
print(f"  - After 1 mean free time, radiation should propagate ~1 mean free path")
print(f"\nFinal results at t = 1.0τ:")
