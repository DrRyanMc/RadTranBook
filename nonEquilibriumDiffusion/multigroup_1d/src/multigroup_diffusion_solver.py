...
from ...shared.src.diffusion_operator_solver import (
...
class MultigroupDiffusionSolver1D:
    ...
    def solve_for_kappa(self, T_star: np.ndarray, xi_g_list: List[np.ndarray],
        ...
        If use_preconditioner=True, uses LMFGK **left-multiplier** C and solves
            (C·B) κ = C·rhs

        Robust logging across SciPy versions:
          - Uses callback_type='pr_norm' so the callback is invoked once per GMRES inner iteration,
            giving a reliable iteration count and preconditioned residual history.

        Stores on the instance:
            self.gmres_precond_resids : list of pr_norm values per iteration (length = iterations)
            self.gmres_true_resids    : list containing ONLY the final true residual (normalized)
        ...
        kappa_initial = np.zeros(self.n_cells)
        ...
        if verbose:
            ...
            x = np.random.rand(self.n_cells)
            y = np.random.rand(self.n_cells)
            a, b = 0.37, -1.12
            Bx = matvec(x); By = matvec(y); Baxy = matvec(a*x + b*y)
            lin_err_B = np.linalg.norm(Baxy - (a*Bx + b*By)) / (np.linalg.norm(Baxy) + 1e-30)
            print(f"  B linearity error: {lin_err_B:.3e} (should be ~0)")
            ...
            for i in range(min(5, self.n_cells)):
                ei = np.zeros(self.n_cells); ei[i] = 1.0
                Bei = matvec(ei)
            ...
            if use_preconditioner:
                print("  Using LMFG (Linear Multifrequency Gray) preconditioning")

        C_operator = None
        if use_preconditioner:
            C_operator = self.create_lmfg_preconditioner(T_star, verbose=verbose)
            if verbose:
                test_zero_C = C_operator.matvec(np.zeros(self.n_cells))
                print(f"    C·0: max = {np.max(np.abs(test_zero_C)):.3e} (should be ~0, verifies homogeneous BCs)")
                x = np.random.rand(self.n_cells); y = np.random.rand(self.n_cells)
                a, b = 0.37, -1.12
                Cx = C_operator.matvec(x); Cy = C_operator.matvec(y); Caxy = C_operator.matvec(a*x + b*y)
                lin_err_C = np.linalg.norm(Caxy - (a*Cx + b*Cy)) / (np.linalg.norm(Caxy) + 1e-30)
                print(f"    C linearity error: {lin_err_C:.3e} (should be ~0)")
        ...
        if use_preconditioner:
            ...
            Aop = LinearOperator((self.n_cells, self.n_cells), matvec=CB_matvec)
    ...
    def compute_gray_operator_coefficients(self, T_star: np.ndarray, 
        ...
        """
        Compute gray-weighted absorption and diffusion coefficients (Eq 8.98, 8.100).
        
        ⟨σ_a⟩ = Σ_g σ*_{a,g} · λ̃_g
        ⟨D⟩ = Σ_g D_g(T_*) · λ̃_g    (arithmetic mean for cell centers)
        
        Note: Harmonic mean is applied at faces in gray_diffusion_coeff(), not here.
        
        Parameters:
        -----------
        T_star : ndarray
            Temperature at linearization point (n_cells,)
        lambda_tilde : ndarray
            Gray weights (n_groups, n_cells)
        
        Returns:
        --------
        sigma_a_gray : ndarray
            Gray-weighted absorption coefficient (n_cells,)
        D_gray : ndarray
            Gray-weighted diffusion coefficient (n_cells,)
        """
        # Compute ⟨σ_a⟩ = Σ_g σ*_{a,g} · λ̃_g
        sigma_a_gray = np.sum(self.sigma_a * lambda_tilde, axis=0)
        
        # Compute ⟨D⟩ = Σ_g D_g(T_*) · λ̃_g (arithmetic mean for cell centers)
        # Harmonic mean is applied later at faces in gray_diffusion_coeff()
        D_gray = np.zeros(self.n_cells)
        r_centers = self.solvers[0].r_centers  # All solvers have same grid
        
        for g in range(self.n_groups):
            # Get diffusion coefficient from the base function
            # Access the diffusion_coeff_func stored in the solver
            D_g_values = np.zeros(self.n_cells)
            for i in range(self.n_cells):
                # Try to call diffusion_coeff_func - it may have extra parameters for flux limiting
                try:
                    D_g_values[i] = self.solvers[g].diffusion_coeff_func(T_star[i], r_centers[i])
                except TypeError:
                    # If flux limiting, need phi_guess - use stored values
                    # For preconditioner, we approximate without flux limiting
                    # Extract base diffusion if possible, otherwise use constant
                    D_g_values[i] = 1.0 / (3.0 * self.sigma_a[g, i]) if self.sigma_a[g, i] > 0 else 0.1
            
            # Debug: check D_g values and weights
            if verbose:
                print(f"    DEBUG Group {g}: D_g[0]={D_g_values[0]:.6e}, λ̃_g[0]={lambda_tilde[g,0]:.6e}")
            
            D_gray += D_g_values * lambda_tilde[g, :]
        
        return sigma_a_gray, D_gray
    
    def create_lmfg_preconditioner(self, T_star: np.ndarray,
                                   verbose: bool = False,
                                   m_mode: str = "identity") -> LinearOperator:
        """
        Create Linear Multifrequency Gray (LMFGK) left-multiplier C (Eq 8.99-8.102).
        
        This builds the LMFGK left-multiplier used to form the preconditioned operator (C·B):
            C = I + ⟨σ_a⟩ · H^{-1} · (1-f)
        
        where H is the gray operator:
            H = -∇·⟨D⟩∇ + ⟨σ_a⟩(1-f) + 1/(c·Δt)
        
        Application to residual r:
            δκ ≈ C·r = r + ⟨σ_a⟩·U 
        
        Parameters:
        -----------
        T_star : ndarray
            Temperature at linearization point
        verbose : bool
        ...
        precond_operator : LinearOperator
            Operator C used as a left-multiplier: solve (C·B)κ = C·rhs
        """
        f = self.fleck_factor
        nu = 1.0 - f
        nu_floor = 1e-10
        ...
        if np.max(np.abs(nu)) < nu_floor:
            if verbose:
                print("  LMFGK: nu=(1-f) ~ 0 everywhere -> returning identity preconditioner")
        ...
        sigma_a_gray, D_gray = self.compute_gray_operator_coefficients(T_star, lambda_tilde, verbose=verbose)
        ...
        if verbose:
            inv_c_dt = 1.0/(C_LIGHT*self.dt)
            H_react = sigma_a_gray*(1.0 - f) + inv_c_dt
            print(f"    DEBUG: <sigma_a> min/max = {sigma_a_gray.min():.3e} / {sigma_a_gray.max():.3e}")
            print(f"    DEBUG: <D>       min/max = {D_gray.min():.3e} / {D_gray.max():.3e}")
            print(f"    DEBUG: H react   min/max = {H_react.min():.3e} / {H_react.max():.3e}")
        if verbose:
            ...
            print(f"    DEBUG: D_gray[0]={D_gray[0]:.6e}")
            #assert 0 
        
        # Create gray diffusion solver for H operator
        # H = -∇·⟨D⟩∇ + ⟨σ_a⟩(1-f) + 1/(c·Δt)
        
        # IMPORTANT: For stencil consistency with group operators, provide
        # D_gray and sigma_a_gray using the same evaluation approach.
        # The group operators use piecewise constant (cell-centered) values
        # with harmonic means at faces.
        
        # Create coefficient functions that match group operator behavior
        r_centers = self.solvers[0].r_centers
        r_faces = self.solvers[0].r_faces
        dr_cells = r_centers[1:] - r_centers[:-1]  # Cell widths
        
        def gray_diffusion_coeff(T, r):
            """
            Evaluate gray diffusion coefficient at position r.
            Uses piecewise constant with harmonic mean at faces.
            """
            # Check if r is at a face (between cells) - use harmonic mean
            # Otherwise use piecewise constant from nearest cell
            ...
            # Check if r is at an interior face
            for i in range(1, len(r_faces) - 1):
                if abs(r - r_faces[i]) < tol:
                    # At face between cells i-1 and i - use harmonic mean
                    D_left = D_gray[i-1]
                    D_right = D_gray[i]
                    dx_left = r_faces[i] - r_centers[i-1]
                    dx_right = r_centers[i] - r_faces[i]
                    dx_total = dx_left + dx_right
                    
                    # Harmonic mean
                    denom = dx_left / max(D_left, 1e-100) + dx_right / max(D_right, 1e-100)
                    return dx_total / max(denom, 1e-100)
            
            # Not at a face - find which cell contains r and return piecewise constant
            for i in range(len(r_centers)):
                if i == 0:
                    if r < r_centers[i]:
                        return D_gray[i]
                elif i == len(r_centers) - 1:
                    if r >= r_centers[i-1]:
                        return D_gray[i]
                else:
                    if r_centers[i-1] <= r < r_centers[i]:
                        return D_gray[i-1]
                    elif r_centers[i] <= r < r_centers[i+1]:
                        return D_gray[i]
            
            return D_gray[0]  # Fallback
        
        def gray_absorption_coeff(T, r):
            """
            Evaluate gray absorption coefficient at position r.
            Uses piecewise constant (cell-centered values).
            """
            # Find which cell contains r
            if r <= r_faces[0]:
                idx = 0
            elif r >= r_faces[-1]:
                idx = len(r_centers) - 1
            else:
                # Find cell containing r
                idx = 0
                for i in range(len(r_centers)):
                    if i < len(r_faces) - 1:
                        if r_faces[i] <= r < r_faces[i+1]:
                            idx = i
                            break
        ...
        def precond_matvec(y: np.ndarray) -> np.ndarray:
            """
            Apply the LMFGK **left-multiplier** C to a vector y.

            This implements the operator used in Morel et al. (2007) to form the
            *preconditioned system* (C B) κ = C·rhs, rather than treating C as an
            approximation to B^{-1}.

            We use the simplified, standard form
                C = I + ⟨σ_a⟩ · H^{-1} · (1-f),
            ...
            Notes:
            - H is the gray diffusion operator built above:
                  H = -∇·⟨D⟩∇ + ⟨σ_a⟩(1-f) + 1/(c·Δt)
            - Homogeneous boundary conditions are used inside the gray solve to
              preserve linearity (C·0 = 0).
            """
            # Build m(y) for the LMFGK left-multiplier C = I + <σ_a> H^{-1} m.
            #
            # Two options are supported:
            #   m_mode="identity"   : m(y) = (1-f)·y  (cheap, 1 gray solve per apply)
            #   m_mode="multigroup" : m(y) = (1-f)·Σ_g σ*_{a,g}·A_g^{-1} y  (paper-closer, costs G group solves)
            if m_mode not in ("identity", "multigroup"):
                raise ValueError(f"Unknown m_mode={m_mode!r}. Use 'identity' or 'multigroup'.")
        ...
        if verbose:
            print(f"    Gray solver created with {self.n_cells} cells")
    ...
    def step(self, max_newton_iter: int = 10, newton_tol: float = 1e-6,
        ...
        use_preconditioner : bool
            Use LMFG (Linear Multifrequency Gray) preconditioning for GMRES (default: False)
        max_relative_change : float
            Maximum relative change in temperature per Newton step (default: 1.0 = no damping)
        verbose : bool
        ...
        for newton_iter in range(max_newton_iter):
            ...
            # Solve for κ using GMRES (with optional preconditioning)
            ...
            # Apply step damping if needed
            delta_T = T_new - T_star
            rel_change = np.linalg.norm(delta_T) / (np.linalg.norm(T_star) + 1e-14)
            if rel_change > max_relative_change:
                alpha = max_relative_change / rel_change
                T_new = T_star + alpha * delta_T
                if verbose:
                    print(f"  Step damped: alpha = {alpha:.3f} (rel_change = {rel_change:.3e})")
