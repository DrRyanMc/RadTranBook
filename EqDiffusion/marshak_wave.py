...
def marshak_left_bc(Er, x):
    ...
    # Now implemented via numerical flux, so works with nonlinear corrections
...
def run_marshak_wave():
    ...
    # Enable nonlinear corrections with limiter for stability
    # Limiter prevents NL correction from dominating and causing oscillations
    solver.use_nonlinear_correction = True
    solver.use_secant_derivative = True
    solver.max_newton_iter_per_step = 50  # Use more Newton iterations for convergence
    solver.nonlinear_skip_boundary_cells = 0  # Skip cells near Dirichlet boundaries
    solver.nonlinear_limiter = 0.0  # Limit NL correction to 0% of linear term (effectively no NL correction)
    
    # Initial condition: cold material (low temperature)
    def initial_Er(r):
        T_init = 0.1  # keV (cold but not too small to avoid numerical issues)
...
def plot_marshak_wave(solutions):
    ...
    ax.set_title('Marshak Wave: Temperature Profiles', fontsize=14, fontweight='bold')
    ...
    ax.set_title('Marshak Wave: Radiation Energy Density Profiles', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=11)
    ax.set_xlim(0, solutions[-1][1][-1])
    
    plt.tight_layout()
    plt.savefig('marshak_wave.png', dpi=150, bbox_inches='tight')
    print("\nPlot saved as 'marshak_wave.png'")
    plt.show()
...
def plot_material_properties():
    ...
    plt.tight_layout()
    plt.savefig('marshak_material_properties.png', dpi=150, bbox_inches='tight')
    print("Material properties plot saved as 'marshak_material_properties.png'")
    plt.show()
