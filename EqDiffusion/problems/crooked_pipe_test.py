...
def is_optically_thick(r, z):
    ...
    #ascending thin region
    if r<1.5  and (z>2.5 and z<3.0):
        return False  # optically thin
    #ascending thin region (right)
...
@njit
def rosseland_opacity_crooked_pipe(Er, r, z):
    """
...
@njit
def specific_heat_crooked_pipe(T, r, z):
    """
...
@njit
def material_energy_crooked_pipe(T, r, z):
    """
    ...
    """
    cv = specific_heat_crooked_pipe(T, r, z)
    return cv * T
...
def bc_left_axis(Er_boundary, r_val, z_val, geometry, time=0.0):
    ...
    T_in_cell = temperature_from_Er(Er_boundary)
    return .5, 1.0/(3.0*rosseland_opacity_crooked_pipe(T_in_cell, r_val, z_val)), 0.0
...
def bc_right_open(Er_boundary, r_val, z_val, geometry, time=0.0):
    ...
    T_in_cell = temperature_from_Er(Er_boundary)
    return .5, 1.0/(3.0*rosseland_opacity_crooked_pipe(T_in_cell, r_val, z_val)), 0.0  # Neumann: dEr/dz = 0
...
def bc_bottom_source(Er_boundary, r_val, z_val, geometry, time=0.0):
    ...
    Time-dependent: Can ramp up/down source with time parameter
    """
    T_in_cell = temperature_from_Er(Er_boundary)
    T_init = 0.01  # keV
    if r_val < 0.5:
        ...
        # Example: could make source time-dependent here, e.g.:
        # source_multiplier = min(1.0, time / 1.0)  # ramp up over 1 ns
        return .5, 1.0/(3.0*rosseland_opacity_crooked_pipe(T_in_cell, r_val, z_val)), Er_source/2
    else:
        # vacuum elsewhere
        return .5, 1.0/(3.0*rosseland_opacity_crooked_pipe(T_in_cell, r_val, z_val)), 0.0  # Neumann: dEr/dz = 0
...
def plot_material_properties(solver, mesh_type='uniform'):
    ...
    for i in range(solver.n1_cells):
        for j in range(solver.n2_cells):
            r = r_centers[i]
            z = z_centers[j]
            opacity_field[i, j] = rosseland_opacity_crooked_pipe(0.0, r, z)
            cv_field[i, j] = specific_heat_crooked_pipe(0.0, r, z)
    ...
    plt.suptitle('Crooked Pipe Material Properties', fontsize=14, fontweight='bold')
    plt.tight_layout()
    filename = f'crooked_pipe_materials_{mesh_type}.png'
    plt.savefig(filename, dpi=150, bbox_inches='tight')
    print(f"Saved: {filename}")
    plt.close()


def plot_solution(solver, title="Crooked Pipe Solution", mesh_type='uniform'):
    ...
    # Plot temperature
    im1 = ax1.pcolormesh(Z, R, T_2d, shading='auto', cmap='RdYlBu_r')
    ax1.set_xlabel('z (cm)', fontsize=12)
    ax1.set_ylabel('r (cm)', fontsize=12)
    ...
    plt.tight_layout()
    filename = f'crooked_pipe_solution_{mesh_type}.png'
    plt.savefig(filename, dpi=150, bbox_inches='tight')
    print(f"Saved: {filename}")
    plt.show()


def plot_cross_sections(solver, mesh_type='uniform'):
    ...
    # Radial profiles at different z
    ax1 = axes[0, 0]
    z_values = [0.5, 2.0, 3.5, 5.0]
    colors = ['blue', 'red', 'green', 'purple']
    for z_val, color in zip(z_values, colors):
        j = np.argmin(np.abs(z_centers - z_val))
        ax1.plot(r_centers, T_2d[:, j], color=color, linewidth=2, label=f'z = {z_centers[j]:.2f} cm')
    ...
    # Axial profiles at different r
    ax2 = axes[0, 1]
    r_values = [0.25, 0.75, 1.25, 1.75]
    for r_val, color in zip(r_values, colors):
        i = np.argmin(np.abs(r_centers - r_val))
        ax2.plot(z_centers, T_2d[i, :], color=color, linewidth=2, label=f'r = {r_centers[i]:.2f} cm')
    ...
    # Log-scale radial profiles
    ax3 = axes[1, 0]
    for z_val, color in zip(z_values, colors):
        j = np.argmin(np.abs(z_centers - z_val))
        ax3.semilogy(r_centers, T_2d[:, j], color=color, linewidth=2, label=f'z = {z_centers[j]:.2f} cm')
    ...
    # Log-scale axial profiles
    ax4 = axes[1, 1]
    for r_val, color in zip(r_values, colors):
        i = np.argmin(np.abs(r_centers - r_val))
        ax4.semilogy(z_centers, T_2d[i, :], color=color, linewidth=2, label=f'r = {r_centers[i]:.2f} cm')
    ...
    plt.suptitle('Crooked Pipe Cross Sections', fontsize=14, fontweight='bold')
    plt.tight_layout()
    filename = f'crooked_pipe_cross_sections_{mesh_type}.png'
    plt.savefig(filename, dpi=150, bbox_inches='tight')
    print(f"Saved: {filename}")
    plt.show()


def plot_fiducial_points_history(times, fiducial_temps, mesh_type='uniform'):
    """
    ...
    mesh_type : str
        Type of mesh ('refined' or 'uniform')
    """
    ...
    plt.tight_layout()
    filename = f'crooked_pipe_fiducial_history_{mesh_type}.pdf'
    show(filename, close_after=True)
    plt.show()


def plot_colormap_at_time(solver, time_value, save_prefix='crooked_pipe', first_one=False):
    """
    ...
    #make the figure wider than tall in proprtion to the domain which is 7 x 2 cm with 250% extra space for the colorbar
...
def main(output_times=None, use_refined_mesh=False, dt_initial=1e-3, dt_max=10.0, dt_increase_factor=1.1):
    ...
    if output_times is None:
        output_times = [1.0, 5.0, 10.0,100.0,200,500.0,1000.0]
    ...
    if use_refined_mesh:
        ...
        for r in r_faces:
            plt.plot([0,7],[r, r], color='black', linewidth=0.25, alpha=0.5)
        for z in z_faces:
        ...
        show("crooked_pipe_refined_mesh.pdf", close_after=True)
        solver = RadiationDiffusionSolver2D(
            ...
            max_newton_iter=30,
            newton_tol=1e-6,
            rosseland_opacity_func=rosseland_opacity_crooked_pipe,
            specific_heat_func=specific_heat_crooked_pipe,
            material_energy_func=material_energy_crooked_pipe,
        ...
        solver = RadiationDiffusionSolver2D(
            coord1_min=0.0,
            coord1_max=2.0,
            n1_cells=60,      # radial cells
            coord2_min=0.0,
            coord2_max=7.0,
            n2_cells=210,     # axial cells
            geometry='cylindrical',
            dt=dt,
            max_newton_iter=30,
            newton_tol=1e-6,
    ...
    plot_material_properties(solver, mesh_type=mesh_type)
    ...
    while t < t_final:
        ...
        for output_t in output_times:
            if output_t not in output_times_saved and abs(t - output_t) < solver.eps:
                ...
                plot_colormap_at_time(solver, t, save_prefix=f'crooked_pipe_{mesh_type}', first_one=first_one)
    ...
    if t_final not in output_times_saved:
        ...
        plot_colormap_at_time(solver, t_final, save_prefix=f'crooked_pipe_{mesh_type}', first_one=first_one)
    ...
    r_centers, z_centers, Er_2d = solver.get_solution()
    T_2d = temperature_from_Er(Er_2d)
    npz_filename = f'crooked_pipe_solution_{mesh_type}_{solver.n1_cells}x{solver.n2_cells}.npz'
    np.savez(npz_filename,
             ...
             T_2d=T_2d,
             times=times,
             fiducial_temps=fiducial_temps)
    print(f"Saved: {npz_filename}")

    # Plot solution
    print("\nPlotting solution...")
    plot_solution(solver, f"Crooked Pipe Solution (t = {t:.3f} ns)", mesh_type=mesh_type)
    plot_cross_sections(solver, mesh_type=mesh_type)
    
    # Plot fiducial point history
    print("\nPlotting fiducial point temperature history...")
    plot_fiducial_points_history(times, fiducial_temps, mesh_type=mesh_type)
    ...
    print("\n" + "="*70)
    print("Crooked Pipe test completed successfully!")
    print("="*70)
