#!/usr/bin/env python3
"""
Test with linear diffusion and analytical Gaussian solution
...
def test_linear_gaussian():
    """
    Test solver against analytical Gaussian diffusion solution.
    """
    print("="*70)
    print("Linear Gaussian Diffusion Test")
    ...
    k_coupling = 1.0e-5  # Much smaller for stability
    ...
    n_steps = 500  # Just 1 step for debugging
    t_final = dt * n_steps
    ...
    solver = RadiationDiffusionSolver(
        ...
        dt=dt, max_newton_iter=50, newton_tol=1e-6,  # Relax tolerances for debugging
    ...
    # Temperature - Initial
    ax = axes[0, 0]
    ax.plot(r, T_init, 'b-', linewidth=2, label='Numerical')
    ...
    # Er - Initial
    ax = axes[0, 1]
    ax.plot(r, Er_init, 'b-', linewidth=2, label='Numerical')
    ...
    # Temperature - Final
    ax = axes[1, 0]
    ax.plot(r, T_final, 'b-', linewidth=2, label='Numerical')
    ...
    # Er - Final
    ax = axes[1, 1]
    ax.plot(r, Er_final, 'b-', linewidth=2, label='Numerical')
    ...
    plt.tight_layout()
    plt.savefig('test_linear_gaussian.png', dpi=150, bbox_inches='tight')
    print(f"\nPlot saved as 'test_linear_gaussian.png'")
    plt.show()
...
if __name__ == "__main__":
    solver, rel_err_Er, rel_err_T = test_linear_gaussian()
    
    print("\n" + "="*70)
    print("Linear Gaussian diffusion test completed!")
    print("="*70)
