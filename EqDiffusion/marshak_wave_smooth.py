...
def run_marshak_wave_smooth():
    ...
    # Disable nonlinear corrections temporarily for debugging
    solver.use_nonlinear_correction = False
    solver.use_secant_derivative = False
    solver.max_newton_iter_per_step = 1  # More iterations for convergence
    solver.nonlinear_skip_boundary_cells = 1  # Skip boundary cells for stability
    solver.nonlinear_limiter = 1.0  # Optimal limiter (30% of linear term - verified stable)
    
    print(f"\nSolver configuration:")
    print(f"  Nonlinear corrections: {solver.use_nonlinear_correction}")
    print(f"  Nonlinear limiter: {solver.nonlinear_limiter}")
    print(f"  Max Newton iterations: {solver.max_newton_iter_per_step}")
    print(f"  Skip boundary cells: {solver.nonlinear_skip_boundary_cells}")
