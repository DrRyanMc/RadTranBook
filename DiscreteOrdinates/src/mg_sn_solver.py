...
try:  # Support both ``import mg_sn_solver`` and package-style imports.
    from .compton import operator_split_compton_update
    from .kompaneets import operator_split_kompaneets_update
except ImportError:
    from compton import operator_split_compton_update
    from kompaneets import operator_split_kompaneets_update

from . import sn_solver
...
def mg_temp_solve_dmd_inc(
    ...
    compton_induced_mode="planck",
    compton_tolerance=1e-10,
    ...
    compton_matrix_func : callable or None
        When supplied, activates a local, operator-split full-Boltzmann
        Compton update after the ordinary transport/material update.  It is
        called as ``compton_matrix_func(T)`` and returns the macroscopic
        group-transfer matrices defined by :class:`compton.ComptonMatrices`.
        The usual ``scat_funcs`` remain the physical angle-scattering opacity
        in the transport equation.
    compton_induced_mode : {"nonlinear", "planck", "lagged", "wien"}
        Treatment of induced scattering in the operator-split Compton solve.
        ``"planck"`` is the robust default used by McGraw et al.;
        ``"nonlinear"`` Picard-iterates the end-of-step scalar intensity in
        the induced terms.
    compton_tolerance, compton_max_iterations, compton_relaxation
        Controls for the local nonlinear material/Compton iteration.
    kompaneets_options : dict or None
        Activates a local linear Kompaneets frequency split instead of the
        full-Boltzmann matrix update.  Required keys are
        ``energy_edges_kev``, ``group_mean_energy_kev``, and
        ``thomson_opacity``.  Optional keys are forwarded to
        :func:`kompaneets.operator_split_kompaneets_update`.
    ...
    while t_current < tfinal:
        ...
        # Operator-split full-Boltzmann Compton frequency redistribution.
        # The transport solve above used scat_funcs only for angle changes;
        # this local update changes photon energy groups and deposits the
        # equal-and-opposite Compton heating into e.
        if compton_matrix_func is not None:
            phi_g, psi_g, e, T, _ = operator_split_compton_update(
                phi_g, psi_g, e, T, dt, c, invEOS, Bg_funcs,
                compton_matrix_func,
            ...
            phi_g, psi_g, e, T, _ = operator_split_kompaneets_update(
                phi_g, psi_g, e, T, dt, c, invEOS,
                **kompaneets_options,
            )
...
def mg_temp_solve_compton_dmd_inc(*args, compton_matrix_func, **kwargs):
    """Multigroup S_N solver with operator-split full-Boltzmann Compton.

    This is the explicitly named Compton variant of
    :func:`mg_temp_solve_dmd_inc`.  Its positional arguments are identical to
    that solver's; pass a temperature-dependent CSK-style group-matrix
    provider as ``compton_matrix_func``.  The regular ``scat_funcs`` argument
    is still used for physical direction-changing scattering during transport.
    """
    return mg_temp_solve_dmd_inc(
        *args, compton_matrix_func=compton_matrix_func, **kwargs)
