...
def _sweep_all_sph_ld(
    ...
    #
    #   Use the slab-like LD solve (sweep right-to-left).  The lumped LD
    #   mass per node is Δr/2 (NOT Δr), so the removal and source terms
    #   carry a ½ factor consistent with the ½ streaming coefficients:
    #       a00 = 0.5 + ½·dz·sh_l,  a01 = -0.5
    #       a10 = 0.5,              a11 =  0.5 + ½·dz·sh_r
    #       rhs0 = ½·dz·Q_l,        rhs1 = ½·dz·Q_r + g_in
    #   With this scaling the starting-direction solve is exact for
    #   solutions that are linear in r (and still preserves constants).
    #
    ...
    for j in range(I - 1, -1, -1):
        ...
        a00g = 0.5 + 0.5 * dz * sh_l
        a01g = -0.5
        a10g = 0.5
        a11g = 0.5 + 0.5 * dz * sh_r
        rhs0g = 0.5 * dz * Ql
        rhs1g = 0.5 * dz * Qr + g_in
